...
def clean_data(dataset):
    ...
    if not exists(clean_text_path):
        ...
        with open(join(get_data_path(), 'corpus', dataset + '_sentences.txt'), encoding='latin-1') as f:
        ...
        corpus_str = '\n'.join(clean_docs)
        f = open(clean_text_path, 'w', encoding='latin-1')
        f.write(corpus_str)
        f.close()
    f = open(clean_text_path, 'r', encoding='latin-1')
    lines = f.readlines()
    min_len = 10000
    aver_len = 0
    max_len = 0
