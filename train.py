...
def train(train_data, val_data, saver):
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        accuracy_res = {"Train":[], "Val":[]}
        with torch.no_grad():
            train_loss, preds_train = model(pyg_graph, train_data)
            ...
            print("Train Results: ...")
            accuracy_res["Train"].append(eval_res_train["accuracy"])
            pprint(eval_res_train)
            ...
            print("Val Results: ...")
            accuracy_res["Val"].append(eval_res_val["accuracy"])
            pprint(eval_res_val)
